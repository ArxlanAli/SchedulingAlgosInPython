...
def returnIndexOfLoweseOrder():
    min = returnFirstOrder()
    for i in range(returnFirstOrder() + 1,len(order)):
        if order[i] < order[min] and processComplete[i] != 1:
            min = i
...
for i in range(0,n):
    order.insert(i,int(raw_input("Enter the order of the processes:")))
...
waiting.insert(0,0)
for i in range(0,n):
    proces = returnIndexOfLoweseOrder()
